...
# Reading in RDS is much quicker than CSV
...
# times = 10 to call each function 10 times
...
# Vectorizing this code to make it faster 
x <- rnorm(10)
x2 <- numeric(length(x))
for(i in 1:10)
    x2[i] <- x[i] * x[i]
...
# The matrix is much faster because all values in a matrix object must be the same data type
microbenchmark(mat[, 1], df[, 1])

# Which is faster, mat[1, ] or df[1, ]? 
# The matrix is way much faster because the values of a column of a data frame must be the same data type, whereas that of a row doesn't have to be.
microbenchmark(mat[1, ], df[1, ])
